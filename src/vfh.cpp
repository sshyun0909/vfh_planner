...
public:
  ...
  scan_updated_(false), robot_reached_(false), robot_aligned_(false), sub_goal_reached_(true)
  {
    ...
    scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>("/scan", 10, std::bind(&VFH::scan_callback, this, std::placeholders::_1));
  ...
  bool calculate_pose(tf2::Transform &robot_transform_) {
    try {
      ...
      transform_stamped = tf_buffer_->lookupTransform(goal_.header.frame_id, robot_frame_, tf2::TimePointZero);     // tf2 버퍼를 사용하여 두 프레임간 변환을 찾음
      tf2::fromMsg(transform_stamped.transform, robot_transform_);                                                  // geometry_msg형식을 tf2형식으로 변환
    ...
    for(size_t i = 0; i < scan_msg_->ranges.size(); ++i) {
      ...
      if((angle_deg >= -180 && angle_deg <= -90) || (angle_deg >= 90 && angle_deg <= 180)) {
        if(scan_msg_->ranges[i] < obstacle_th_) {
          // RCLCPP_INFO(this->get_logger(), "%.3fm, Obstacle Detected at angle %.3f degrees", scan_msg_->ranges[i], angle_deg);
          return true;
        }
      }
    }
  ...
  geometry_msgs::msg::Twist move_robot() {
    geometry_msgs::msg::Twist cmd_vel;
    double distance_to_goal = std::hypot(goal_.pose.position.x - robot_transform_.getOrigin().x(), goal_.pose.position.y - robot_transform_.getOrigin().y());
    ...
    if(!robot_aligned_ && robot_reached_) {
      ...
      RCLCPP_INFO(this->get_logger(), "Align velocity: speed: %.3f, angular: %.3f", cmd_vel.linear.x, cmd_vel.angular.z);
      ...
      RCLCPP_INFO(this->get_logger(), "Move velocity: speed: %.3f, angular: %.3f", cmd_vel.linear.x, cmd_vel.angular.z);
    }
    return cmd_vel;
  }
  ...
  geometry_msgs::msg::Twist vfh_velocity() {
    ...
    cmd_vel.linear.x = max_velocity_;
    cmd_vel.angular.z = 0.0;
    RCLCPP_INFO(this->get_logger(), "Align VFH velocity: speed: %.3f, angular: %.3f", cmd_vel.linear.x, cmd_vel.angular.z);

    if(!sub_goal_reached_) {
      cmd_vel.linear.x = max_velocity_;
      cmd_vel.angular.z = sub_yaw_error_;
      RCLCPP_INFO(this->get_logger(), "Move VFH velocity: speed: %.3f, angular: %.3f", cmd_vel.linear.x, cmd_vel.angular.z);
  ...
  void create_histogram() {
    ...
    if(vector_index < 0) {                                                                       // 잘되면 나중에 지워도 될듯?
      vector_index += 360;
    }
    ...
    best_index_ = (left_index_ <= (distance_vector.size() - 1.0 - right_index_)) ? left_index_ : right_index_;
    RCLCPP_INFO(this->get_logger(), "Selected direction: %s, Index: %d", (best_index_ == left_index_) ? "Left" : "Right", best_index_);
    ...
    RCLCPP_INFO(this->get_logger(), "Update Sub Goal: x=%.3f, y=%.3f, yaw=%.3f", sub_goal_x_, sub_goal_y_, sub_goal_.pose.orientation.w);
  ...
  double update_sub_pose() {
    ...
    RCLCPP_INFO(this->get_logger(), "robot_x: %.3f, robot_y: %.3f, robot_yaw: %.3f", robot_x_, robot_y_, robot_yaw_);

    while(sub_yaw_error_ > M_PI) sub_yaw_error_ -= 2.0 * M_PI;
    while(sub_yaw_error_ < -M_PI) sub_yaw_error_ += 2.0 * M_PI;

    RCLCPP_INFO(this->get_logger(), "Sub Goal: x=%.3f, y=%.3f, yaw=%.3f", sub_goal_x_, sub_goal_y_, sub_goal_.pose.orientation.w);
    RCLCPP_INFO(this->get_logger(), "sub_target_yaw: %.3f, sub_yaw_error: %.3f", sub_target_angle_, sub_yaw_error_);
...
private:
  ...
  double sub_goal_angle_, sub_goal_x_, sub_goal_y_, sub_goal_distance_, sub_distance_th_, safety_distance_;
